...
// Read-only mirror of app/(tabs)/calendar.tsx for viewing a family member's
// calendar — same structure, just every hook is passed `targetUserId`
// instead of defaulting to the current user. There's nothing to strip for
// "view only": the day-detail panel below was already a plain read-only
// summary (checks/crosses, no toggles), same as the personal Calendar tab.
...
export default function MemberCalendarScreen() {
  ...
  const selectedDateString = selectedDay
    ...
    : null;
  const { completed: selectedDayCompleted } = useDayPrayerLogs(selectedDateString, userId);
  ...
  return (
    <View className="flex-1 bg-surface">
      <TopAppBar title={name ?? t('tabCalendar')} />
      <ScrollView className="flex-1 px-container-margin" contentContainerStyle={{ paddingBottom: 32, paddingTop: 12 }}>
